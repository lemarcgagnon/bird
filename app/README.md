...
- Library decor remains a server/PHP download flow: the Decor panel lists active library files with server-generated PNG thumbnails, offers a controlled Three.js STL preview modal before download, and can authorize/download them with credits. In development, the modal loads the untouched mirrored STL from `app/images/library` when `app_original_url` is present, then falls back to the high-detail sampled PHP preview JSON. The client still receives the purchased original STL/image file on their computer and imports that local file through the decoration uploader. Uploaded library originals are mirrored locally under `app/images/library` for local/admin previews. Imported STL decor defaults to full-mesh placement with proportions locked and clipping off so the WASM viewer does not silently cut or squash the model.
