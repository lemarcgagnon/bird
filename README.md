...
- Le site PHP suit maintenant une base i18n FR/EN sur `/`, `/pricing`, `/account` et `/admin`, avec conservation de `?lang=`/cookie et labels metier coherents entre vues publiques, compte et back-office.
...
- Les tables admin sont maintenant coherentes sur tout le back-office: tri par colonnes, select `Afficher 10 / 25 / 50 / 100 / Toutes` avec `10` par defaut, et meme comportement dans les tables de detail du modal client.
- `/admin` preserve maintenant mieux la continuite d'usage: la zone de travail garde sa position de scroll apres les interactions internes, au lieu de renvoyer l'utilisateur en haut ou sur une vue parasite.
- Les modals admin ont ete elargis sur desktop et leur focus initial n'entraine plus de deplacement de viewport parasite.
...
- Les emails backend visibles par humain suivent aussi cette passe: email d activation compte, sujets et corps des notifications support client, selon la langue courante de la requete.
...
- Finir la revue responsive/mobile du back-office selon `docs/admin-hig.md`.
