...
- Library decor remains a server/PHP download flow: the Decor panel lists active library files, renders STL previews with Three.js from public sampled preview JSON, and can authorize/download them with credits. The client still receives the STL/image file on their computer and imports that local file through the decoration uploader. `app.js` does not fetch private library files directly into WASM.
