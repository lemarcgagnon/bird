...
- Library decor remains a server/PHP download flow: the Decor panel lists active library files with server-generated PNG thumbnails, offers a high-detail sampled Three.js STL preview modal before download, and can authorize/download them with credits. The client still receives the original STL/image file on their computer and imports that local file through the decoration uploader. Imported STL decor defaults to full-mesh placement with proportions locked and clipping off so the WASM viewer does not silently cut or squash the model. `app.js` does not fetch private library files directly into WASM.
