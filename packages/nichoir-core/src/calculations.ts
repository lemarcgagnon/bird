...
// Port fidèle de src/calculations.js. Divergence : `computeCutList` omet
// la destructuration de `T` (unused dans src, supprimé pour satisfaire TS strict).
...
export function computeCutList(params: Params, derived: DerivedDims): CutList {
  const { slope, floor, ridge, taperX, door, doorW, doorH, doorVar, doorPanel, perch, perchDiam, perchLen } = params;
  ...
  const sideNoteKey = hasTaper
    ...
    : null;
  const sideNoteParams: Record<string, string | number> | null = hasTaper
    ? { v: Math.abs(taperX).toFixed(0) }
    : null;
